...
class Canvas {
  constructor({ container, canvas, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ, onItemClick }) {
  ...
  onTouchUp(e) {
    ...
    const dist = Math.sqrt(Math.pow(endY - this.start, 2) + Math.pow(endX - this.startX, 2));
    const duration = performance.now() - this.startTime;

    if (dist < 5 && duration < 250) {
      this.onClick(endX, endY);
    }
  }
  ...
  onClick(x, y) {
    ...
    // Map relative Y (0 to height) to WebGL Y (viewport.height / 2 to -viewport.height / 2)
    const yWebGL = map(yRel, 0, rect.height, this.viewport.height / 2, -this.viewport.height / 2);

    // Find the media that contains this Y
    const hitMedia = this.medias.find(media => {
      const planeY = media.plane.position.y;
      const halfHeight = media.plane.scale.y / 2;
      return yWebGL >= planeY - halfHeight && yWebGL <= planeY + halfHeight;
    });

    if (hitMedia && this.onItemClickCallback) {
      this.onItemClickCallback(hitMedia.index);
    }
  }
